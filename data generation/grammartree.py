...
def getValueFromInstruction(sent):
    ...
    #if an adjective is present then truth is set to True to activate the correct pipeline 
    truth = False
        ...
        try:
            for x in range (len(tags)):
                if "JJ" in tags[x]:
                    ...
                    #while the word after the adjective is any of these parts of speech they're added to the instruction final
                    while("VBN" in tags[q] or "VBG" in tags[q] or "NN" in tags[q] or "NNS" in tags[q] or "RB" in tags[q] or ("NNS" in tags[q] and "IN" in tags[q + 1])):
                        ...
                        #if an interjection is present then you want to skip over it
                        if("IN" in tags[q + 1]):
            ...
            for x in range (len(tags)):
                if x < len(tags) - 1:
                    if "IN" in tags[x + 1]:
                        ...
                        x = x + 2
                        continue
                #if any of these parts of speech are in the instruction then you want to extract them. The parts of speech can be found: https://repository.upenn.edu/cgi/viewcontent.cgi?article=1603&context=cis_reports
